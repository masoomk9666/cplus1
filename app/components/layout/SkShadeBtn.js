...
const SkShadeBtn = ({ 
  ...
  return (
    <button className={`group relative text-[14px] md:text-[18px] ${bgColorClass} ${textColorClass} px-4 sm:px-5 py-1.5 sm:py-2 rounded-full w-fit overflow-hidden cursor-pointer transition-all duration-300 hover:scale-105 active:scale-95`}>
      {text}
      {/* Inner glow - Responsive */}
      <span className={`absolute -bottom-3 sm:-bottom-4 left-1/2 -translate-x-1/2 ${shadeWidthClass} h-3 sm:h-4 md:h-5 rounded-full ${shadeColorClass} opacity-0 scale-95 blur-sm pointer-events-none transition-all duration-500 ease-in-out group-hover:opacity-100 group-hover:scale-100`}></span>
    </button>
  );
};
