...
const CardCarousel = () => {
  ...
  return (
    <div className="relative w-full overflow-hidden pt-10 md:pt-15 lg:pt-25">
      ...
      <div className="relative w-full h-[200px] sm:h-[250px] md:h-[300px] lg:h-[350px] flex items-center">
        <motion.div
          className="flex gap-0"
          animate={{ x: ["0%", "-50%"] }}
          transition={{
            duration: isMobile ? 30 : isTablet ? 40 : 50,
            ease: "linear",
            repeat: Infinity,
          }}
        >
          {[...images, ...images].map((src, index) => (
            ...
            >
              <img
                src={src}
                alt={`Slide ${index}`}
                className="w-full h-full rounded-lg md:rounded-xl transition-transform cursor-pointer"
                onMouseEnter={(e) => {
                  if (isMobile) {
